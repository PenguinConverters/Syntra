...
/// <summary>
/// Builds an <see cref="IProvider"/> instance with the required dependencies.
...
public interface IProviderBuilder
{
    ...
    /// The decryptor holding the vault key. It is owned by the caller and outlives the provider,
    /// so the provider must not dispose it.
    /// </param>
    void AddDecryptor(Decryptor decryptor);

    /// <summary>
    /// Builds and returns the configured <see cref="IProvider"/> instance.
    /// </summary>
